...
public class NlpService(OpenAIClient client, IOptions<PromptSettings> opts) : INlpService
{
    ...
    public async Task<UserIntentDetected> DetectIntentAsync(Guid correlationId, string rawText)
    {
        ...
        return new UserIntentDetected(
            correlationId,
            intent,
            intent == "transfer" ? new TransferPayload(
            ...
            intent == "billpay" ? new BillPayload(
            ...
            //TODO: its wrong
            intent == "set_goal" ? new GoalPayload(
                Guid.Empty,
            ...
            intent == "schedule_recurring" ? new RecurringPayload(
                Guid.NewGuid(),
                new TransferPayload(
                    root.GetProperty("toAccount").GetString()!,
                    root.GetProperty("bankCode").GetString()!,
            ...
            intent == "feedback" ? new FeedbackPayload(
                root.GetProperty("rating").GetInt32(),
                root.GetProperty("comment").GetString()!
            ) : null,
            intent == "signup" ? new SignupPayload(
